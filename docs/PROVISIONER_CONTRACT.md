...
The provisioner belongs in `score-gp-aws-rds/.score-k8s/10-application-stack.provisioners.yaml`. `score-k8s generate` loads it from that directory together with the application-stack workload specification. Keep any existing `.score-k8s/state.yaml` unchanged.
