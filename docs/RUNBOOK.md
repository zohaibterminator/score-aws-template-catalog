...
Score resource (application-stack.aws)
  → Terraform CR generated by the Score provisioner
  → Flux applies them
...
3. Run the agent in `agents/provisioner-agent`. It reads the catalog, all six contracts, and the policy docs, then pushes a Score provisioner to a review branch in `score-gp-aws-rds`. Supply a pre-existing Kubernetes Secret by name for RDS and production cache. Keep secret values out of Git.
