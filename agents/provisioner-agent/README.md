...
The approval code is a SHA-256 digest of the exact provisioner. A content change changes the code and blocks publishing until the new output is reviewed. Publishing pushes `10-application-stack.provisioners.yaml` to a new `agent/application-stack-*` review branch in `application-stack/.score-k8s`. It does not change `main`, `.score-k8s/state.yaml`, or existing provisioners. The model only chooses the preview or publish tool; it cannot edit the rendered YAML or run shell commands. The harness runs before either tool.
